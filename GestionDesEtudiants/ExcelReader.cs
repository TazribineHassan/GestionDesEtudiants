            ...
            using (OleDbConnection connection = new OleDbConnection(con))
            {
                try
                {
                        ...
                        using (OleDbDataReader dr = command.ExecuteReader())
                        {
                            ...
                            while (dr.Read())
                            {
                                students.Add(new Student(0, new Branch(0, dr.GetString(columns.nom_filiere)), dr.GetString(columns.CNE), dr.GetString(columns.nom), dr.GetString(columns.prenom), dr.GetString(columns.sex), dr.GetString(columns.address), dr.GetDateTime(columns.date_de_naissance), dr.GetDouble(columns.telephone).ToString()));
                            }
